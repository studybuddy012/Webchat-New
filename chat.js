...
import { 
  getFirestore, collection, addDoc, query, orderBy, onSnapshot, 
  doc, setDoc, limit, where, getDocs, writeBatch, startAfter 
} from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
...
/* ================= 3. TOKEN & SESSION LOGIC ================= */
let tokenClient;
function getNewToken(callback) {
    if (typeof google === 'undefined') { alert("Google Library loading..."); return; }
    tokenClient = google.accounts.oauth2.initTokenClient({
        client_id: CLIENT_ID, scope: SCOPES,
        callback: (res) => {
            if (res.access_token) {
                localStorage.setItem('drive_token', res.access_token);
                localStorage.setItem('token_expiry', Date.now() + 3500 * 1000);
                if (callback) callback(res.access_token);
            }
        },
    });
    tokenClient.requestAccessToken();
...
const isTokenValid = () => {
    const t = localStorage.getItem('drive_token'), e = localStorage.getItem('token_expiry');
...
/* ================= 5. CORE CHUNKING LOGIC ================= */

// Function to fetch and prepend older chats
async function loadChatChunk() {
    if (isLoading) return;
    isLoading = true;

    let q = query(messagesRef, orderBy("time", "desc"), limit(CHUNK_SIZE));
    if (lastVisible) q = query(q, startAfter(lastVisible));

    const snap = await getDocs(q);
    if (!snap.empty) {
        // Pointer update for next scroll
        lastVisible = snap.docs[snap.docs.length - 1];
        
        // Agar pehli baar load ho raha hai, toh latest message save karo for real-time listener
        if (!firstMsgLoaded) firstMsgLoaded = snap.docs[0];

        const oldHeight = messageBox.scrollHeight;
        
        // Reverse kyunki hum 'desc' fetch kar rahe hain par UI me 'asc' chahiye
        snap.docs.forEach(docSnap => {
            const data = { id: docSnap.id, ...docSnap.data() };
            const div = document.createElement("div");
            div.id = `msg-${data.id}`;
            div.className = `msg-row ${data.sender === username ? "me-row" : "other-row"}`;
            div.innerHTML = createMessageHTML(data);
            messageBox.prepend(div); // Purani chats upar lagengi
        });

        // Scroll position fix (jump na ho)
        if (oldHeight > 0) messageBox.scrollTop = messageBox.scrollHeight - oldHeight;
        else messageBox.scrollTop = messageBox.scrollHeight;
    }
    isLoading = false;
}

// Real-time listener: Sirf naye aane wale messages ke liye
function startLiveListener() {
    const q = query(messagesRef, orderBy("time", "asc"), startAfter(firstMsgLoaded || 0));
    onSnapshot(q, (snap) => {
        snap.docChanges().forEach(change => {
            const data = { id: change.doc.id, ...change.doc.data() };
            ...
            if (change.type === "added" && !existing) {
                ...
                if (data.sender === otherUser && document.visibilityState === 'visible') markAsSeen();
            } else if (change.type === "modified" && existing) {
                existing.innerHTML = createMessageHTML(data);
            }
        });
    });
}

// Scroll Event: Check if user reached top
messageBox.addEventListener("scroll", () => {
    if (messageBox.scrollTop === 0) loadChatChunk();
});

/* ================= 6. OTHER SYNC & ACTIONS ================= */
onSnapshot(otherStatusRef, (snap) => {
    if (!snap.exists()) return;
    const d = snap.data();
    const statusEl = document.getElementById("user-status"), typingInd = document.getElementById("typing-indicator");
    if (d.isTyping) typingInd.classList.remove("hidden"); else typingInd.classList.add("hidden");
    if (d.state === "online") { statusEl.textContent = "Online"; statusEl.style.color = "#4cd137"; }
    else { statusEl.textContent = `Last seen at ${new Date(d.last_changed).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}`; statusEl.style.color = "#aaa"; }
});
...
/* ================= 7. ATTACHMENTS ================= */
const fileInput = document.getElementById("drive-upload");
document.getElementById("attach-btn").onclick = () => { if (isTokenValid()) fileInput.click(); else getNewToken(() => fileInput.click()); };

fileInput.onchange = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const token = localStorage.getItem('drive_token'), tempId = "temp-" + Date.now();
    
    // UI placeholder for upload
    const div = document.createElement("div");
    div.id = `msg-${tempId}`; div.className = "msg-row me-row";
    div.innerHTML = `<div class="bubble me"><div class="upload-info" id="up-${tempId}">Uploading 0%</div></div>`;
    messageBox.appendChild(div); messageBox.scrollTop = messageBox.scrollHeight;
    ...
    xhr.upload.onprogress = (ev) => {
        const p = Math.round((ev.loaded / ev.total) * 100);
        document.getElementById(`up-${tempId}`).innerText = `Uploading... ${p}%`;
    };
...
/* ================= 8. LIFECYCLE ================= */
const updateStatus = (s) => setDoc(userStatusRef, { state: s, last_changed: Date.now(), isTyping: false }, { merge: true });
window.logout = () => { updateStatus("offline"); localStorage.clear(); location.href = "index.html"; };
